...
curl -X PUT -H "Content-Type: application/json" -d '{"is_pressed": true}' http://localhost:5000/user/$id

echo "Состояние кнопки для пользователя $username успешно изменено на true."
